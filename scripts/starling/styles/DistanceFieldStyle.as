package starling.styles
{
   ...
   public class DistanceFieldStyle extends MeshStyle
   {
      ...
      private function updateVertices() : void
      {
         ...
         var _loc5_:Number = (_shadowOffsetX + 8) / (2 * 8);
         var _loc6_:Number = (_shadowOffsetY + 8) / (2 * 8);
         var _loc8_:uint = uint(_threshold * 255) | uint(_alpha * 255) << 8 | uint(_softness / 2 * 255) << 16 | uint(1 / 8 * 255) << 24;
